...
class WalletApp {
    constructor() {
        ...
        this.assetUrl = (path) => chrome.runtime.getURL(`assets/${path}`);
    ...
    initLoginHandler() {
        ...
        if (loginButton) {
            loginButton.addEventListener('click', () => {
                ...
                // For Twitter inject context, use postMessage
                if (window.location.href.includes('twitterUsername')) {
                    window.parent.postMessage({ type: 'INITIATE_OAUTH' }, '*');
                }
                else {
                    // Direct extension popup context
        ...
        // Listen for messages from parent (Twitter inject)
        window.addEventListener('message', (event) => {
            if (event.data.type === 'OAUTH_COMPLETE') {
                this.hideLoadingState();
                this.handleWalletConnected(event.data.wallet);
            }
            if (event.data.type === 'OAUTH_ERROR') {
                this.hideLoadingState();
                this.showError(event.data.error || 'Authentication failed');
            }
        });
    }
    handleAuthResponse(response) {
        ...
        if (response && response.success) {
            if (response.data.wallet) {
                this.handleWalletConnected(response.data.wallet);
            }
            else {
                // No wallet yet - show import screen
                this.showImportWalletScreen();
            }
        }
        else {
            this.showError((response === null || response === void 0 ? void 0 : response.error) || 'Authentication failed');
        }
    }
    handleWalletConnected(walletData) {
        // Update the wallets array with the authenticated wallet
        this.wallets = [{
                id: walletData.address,
                name: walletData.name || `${walletData.username}'s Wallet`,
                balance: 0,
                usdBalance: '$0.00',
    ...
    `;
        ...
        (_b = document.getElementById('depositButton')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
            console.log('Deposit button clicked!');
            this.showDepositScreen();
        });
        // Network select dropdown
        (_c = document.getElementById('networkSelect')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => {
        ...
        <div class="wallet-header">
          <div class="user-profile">
            <img 
              ...
              src="${((_a = currentWallet.user) === null || _a === void 0 ? void 0 : _a.profile_image_url) || 'default-avatar.png'}" 
              alt="Profile"
            />
            <span class="username">@${((_b = currentWallet.user) === null || _b === void 0 ? void 0 : _b.username) || 'user'}</span>
          </div>
    ...
    `;
        ...
        requestAnimationFrame(() => {
            var _a, _b, _c;
            (_a = document.getElementById('settingsBtn')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
                this.showSettings();
            });
            (_b = document.getElementById('sendBtn')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
                this.handleSend();
            });
            (_c = document.getElementById('receiveBtn')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => {
                this.handleReceive();
            });
        });
    }
    ...
    `;
        ...
        (_a = document.getElementById('backButtonSend')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
            console.log('Back button clicked');
            this.showDashboard();
        });
        (_b = document.getElementById('maxBtn')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
            const amountInput = document.getElementById('sendAmount');
            if (amountInput) {
                amountInput.value = currentWallet.balance.toString();
            }
        });
        (_c = document.getElementById('confirmSendBtn')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => {
            var _a, _b;
            const recipientAddress = (_a = document.getElementById('recipientAddress')) === null || _a === void 0 ? void 0 : _a.value;
            const amount = (_b = document.getElementById('sendAmount')) === null || _b === void 0 ? void 0 : _b.value;
    ...
    `;
        (_a = document.getElementById('successBackToDashboardBtn')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
    ...
    `;
        ...
        // Add event listeners
        (_a = document.getElementById('backButtonDeposit')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
            console.log('Back button clicked from deposit screen');
            this.showDashboard();
        });
        (_b = document.getElementById('copyAddressBtn')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
        ...
        (_c = document.getElementById('backToDashboardBtn')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => {
    ...
    `;
        (_a = this.mainContent) === null || _a === void 0 ? void 0 : _a.appendChild(helpText);
    }
    showRetryButton() {
        var _a;
        ...
        (_a = this.mainContent) === null || _a === void 0 ? void 0 : _a.appendChild(retryButton);
    }
    debugInteractions() {
        document.addEventListener('click', (event) => {
            const target = event.target;
            ...
            if (sendBtn) {
                ...
                const isNearSend = event.clientX >= rect.left - 10 &&
    ...
    `;
      ...
      `;
        ...
        const closeDropdown = (e) => {
            if (!e.target.closest('#networkDropdown') &&
                !e.target.closest('#networkSelect')) {
                dropdown.remove();
    ...
    // Add a new screen for wallet import
    showImportWalletScreen() {
      ...
      <div class="import-container">
        ...
        <div class="import-form" style="width: 100%; max-width: 300px; margin: 0 auto;">
          <h3 style="margin-bottom: 16px; text-align: center; color: white;">Import Your Ethereum Wallet</h3>
          
          <div style="margin-bottom: 24px;">
            <label style="display: block; margin-bottom: 8px; color: rgba(255,255,255,0.7);">Wallet Address</label>
            <input type="text" id="walletAddress" placeholder="0x..." 
              style="width: 100%; padding: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); 
              border-radius: 8px; color: white; font-size: 14px;">
          </div>
          
          <div id="addressError" style="color: #f87171; margin-bottom: 16px; font-size: 14px; display: none;"></div>
          ...
          <p style="text-align: center; color: rgba(255,255,255,0.5); font-size: 12px; margin-top: 16px;">
            Your private keys remain in your possession.<br>We only store your public address.
          </p>
        </div>
      </div>
    `;
        // Add event listeners
        (_a = document.getElementById('importButton')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
    ...
    handleWalletImport() {
        ...
        if (!address.match(/^0x[a-fA-F0-9]{40}$/)) {
            errorDiv.textContent = 'Invalid Ethereum wallet address format';
            errorDiv.style.display = 'block';
            return;
        }
        errorDiv.style.display = 'none';
        this.showLoadingState();
        // Get Twitter username from URL or context
        ...
        chrome.runtime.sendMessage({
            ...
            this.hideLoadingState();
            if (response && response.success) {
                this.handleWalletConnected(response.wallet);
            }
            else {
                this.showError((response === null || response === void 0 ? void 0 : response.error) || 'Failed to import wallet');
            }
        });
    }
}
